\begin{slide}{Modeling EXAFS and the EXAFS Equation}

 EXAFS can be modeled using the {\BlueEmph{EXAFS Equation}}:
  \vspace{-1mm}
  ...
  where:
  \begin{itemize}
  \item the sum is over {\RedEmph{shells}}  of atoms surrounding the
    absorbing atom.
  \item $\Red{f(k)}$ and $\Red{\delta(k)}$ are
    {\RedEmph{photo-electron scattering properties}}
